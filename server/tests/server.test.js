...
describe('GET /todos', () => {
    it('should get all todos', (done) => {
        request(app)
            ...
            .expect(200)
            .expect((res) => {
                expect(res.body.todos.length).toBe(1); // this is the single todo mady by the user users[0]
            })
            .end(done);
    });
});
...
describe('PATCH /todos/:id', () => {
    ...
    it('should clear completedAt when todo is not completed', (done) => {
        var hexId = todos[1]._id.toHexString();
        var text = 'Brand new updated text';
...
describe('POST /users/login', () =>  {
    it('should login user and return auth token', (done) => {
        request(app)
            ...
            .expect((res) => {
                expect(res.headers['x-auth']).toBeTruthy();
                expect(users[1].email).toBe(res.body.email); // check if it is userTwo@example.com
            })
            .end((err, res) => {
                if (err) {
                    return done(err);
